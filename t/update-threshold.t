...
delete $got_levels->{ok};
delete $got_levels->{severe};

print STDERR "update config\n";

#now greatly bump up the threshold and make sure we get an 'ok'
$config->{'.multimodule'}->{config}->{MultiModule}->{thresholds}->{'local.memory.vss_of_box_physical'} = {
    levels => [
        {   name => 'failsafe',
            threshold => 0.6, 
        },{ name => 'severe',
